...
delete_max' (UserNode (elm, _, _, [(_, left_ret), (UserLeaf, _)])) = left_ret
delete_max' (UserNode (elm, node, _, [(left_tree, left_ret), (right_tree, right_ret)])) =
    node [left_ret, delete_max' right_tree]

delete' :: Ord e => e -> UserTree e -> Update e
delete' _ UserLeaf = leaf
delete' e (UserNode (elm, node, rep_node, [(left_tree, left_ret), (right_tree, right_ret)]))
